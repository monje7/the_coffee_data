...
export const guardarMuestra = async (req, res) => {
    try {
        ...
        let sql =  `INSERT INTO muestras( fecha_creacion, cantidad, estado, cafes_id) VALUES (?,?,?,?)`

        const [rows] = await pool.query(sql, [ data.fecha, data.cantidad,data.estado,data.cafe ]); 
